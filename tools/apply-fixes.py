...
sub(
    ...
    "     Watching only sees the minutes a browser was open. The gateway keeps a\n"
    "     day of past packages, each still carrying all five signatures, so the\n"
    "     missing hours can be read back instead of guessed at.\n"
    "\n"
    "     Read on exact hour boundaries, and stored in the same hourly buckets the\n"
    "     live watch fills. That makes it repeatable — the same hour always gives\n"
    "     the same answer — and cumulative, since an hour already known is never\n"
    "     fetched twice. Sampling nine moving points instead produced a different\n"
    "     number on every press, which is noise wearing the costume of a metric. */\n"
    "  async scan24h(sym){\n"
    ...
    "    const buckets = this.spreadMem[sym] || (this.spreadMem[sym] = {});\n"
    "    const missing = [];\n"
    "    for(let h = nowH - 23; h < nowH; h++) if(!buckets[h]) missing.push(h);\n"
    "    const batch = missing.slice(0, SCAN_BATCH);\n"
    "\n"
    "    for(let i = 0; i < batch.length; i++){\n"
    "      if(!this.alive) break;\n"
    "      if(out) out.textContent = 'reading hour ' + (i+1) + ' of ' + batch.length + '…';\n"
    "      try {\n"
    "        const r = await fetch(HIST_GW + (batch[i] * 3600000));\n"
    "        if(!r.ok) throw new Error('HTTP ' + r.status);\n"
    ...
    "        buckets[batch[i]] = {\n"
    "          max: med ? ((Math.max(...vals) - Math.min(...vals))/med)*10000 : 0,\n"
    "          n: 1, read: 1\n"
    "        };\n"
    "      } catch(e){ /* an hour the gateway will not serve stays unknown */ }\n"
    "    }\n"
    "\n"
    "    try { localStorage.setItem(SPREAD_KEY, JSON.stringify(this.spreadMem)); } catch(e){}\n"
    "    this.scanning = false;\n"
    "    if(btn) btn.disabled = false;\n"
    "    if(this.alive) this.paintScan();\n"
    "  }\n"
    "  /* The widest gap across the last 24 hours, from every hour known — whether\n"
    "     that hour was watched live or read back — plus how many of the 24 those\n"
    "     are, so the number is never mistaken for more coverage than it has. */\n"
    "  gap24(sym){\n"
    "    const nowH = Math.floor(Date.now()/3600000);\n"
    "    const buckets = (this.spreadMem && this.spreadMem[sym]) || {};\n"
    "    let max = null, worst = null, known = 0;\n"
    "    for(let h = nowH - 23; h <= nowH; h++){\n"
    "      const b = buckets[h];\n"
    "      if(!b) continue;\n"
    "      known++;\n"
    "      if(max === null || b.max > max){ max = b.max; worst = h; }\n"
    "    }\n"
    "    return known ? {max, worst, known} : null;\n"
    "  }\n"
    "  paintScan(){\n"
    "    const out = document.getElementById('pop-scan-out');\n"
    "    const btn = document.getElementById('pop-scan-btn');\n"
    "    if(!out || this.scanning) return;\n"
    "    const g = this.gap24(this.selected);\n"
    "    const left = g ? 24 - g.known : 24;\n"
    "    if(btn) btn.textContent = left > 0 ? 'READ ' + Math.min(left, SCAN_BATCH) + ' MORE HOURS' : 'ALL 24 HOURS READ';\n"
    "    if(btn) btn.disabled = left <= 0;\n"
    "    if(!g){\n"
    "      out.textContent = 'no hour known yet — read the last 24 hours back from the gateway';\n"
    "      return;\n"
    "    }\n"
    "    const hour = (h) => new Date(h*3600000).toISOString().slice(5,16).replace('T',' ').replace('-','/');\n"
    "    out.innerHTML =\n"
    "      '<b style=\"color:#FFE3E3;font-weight:500\">' + g.max.toFixed(2) + ' bps</b>' +\n"
    "      ' widest · ' + g.known + ' of 24 hours known' +\n"
    "      (g.max > 0 ? ' · worst hour ' + hour(g.worst) + ' UTC' : '') +\n"
    "      (left > 0 ? ' · ' + left + ' still unread' : ' · complete');\n"
    "  }\n"
